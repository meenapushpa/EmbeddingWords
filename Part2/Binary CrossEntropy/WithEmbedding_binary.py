...
model = Sequential()
e = Embedding(vocab_size, 100, input_length=seq_len, trainable=True) # trainable=True
...
print('[Manual run binary_crossentropy] With Embedding accuracy: %f' % acc)
...
False_data.to_excel("WithEmbeddingConsolidate_binary.xlsx",index=False)

loss,accuracy=model.evaluate(X_val,y_val,verbose=1)
print('[INFO] binary_crossentropy Evaluate method With Embedding accuracy: %f' % accuracy)

accuracyvalues = K.mean(K.equal(y_val, K.round(sequentialpredict)))
print('[K-mean binary_crossentropy] With Embedding accuracy: %f' % accuracyvalues)

#calculate the values of accuracy,precison,recall,f1 score
precision = precision_m(y_val, sequentialpredict)
recall = recall_m(y_val, sequentialpredict)
f1_score = f1_m(y_val, sequentialpredict)
print('[INFO] binary_crossentropy With Embedding f1_score: %f' % f1_score)
print('[INFO] binary_crossentropy With Embedding precision: %f' % precision)
print('[INFO] binary_crossentropy With Embedding recall: %f' % recall)
